...
public:
    ...
    public:
        enum {
			...
			XN   = 1 << 0, // Execute never. Stops execution of page
            V    = 1 << 1, // Bit 1 é 1 quando valido
            B    = 1 << 2, // Bufferable
            C    = 1 << 3, // Cacheable
            AP1  = 1 << 4, // Access Permission
            AP2  = 1 << 5, // Access Permission
            TEX1 = 1 << 6, // Type Extension
            TEX2 = 1 << 7, // Type Extension
            TEX3 = 1 << 8, // Type Extension
            APX  = 1 << 9, // Access
            S    = 1 <<10, // Shareable. 0 - non-shareable, 1 - shareable.
            nG   = 1 <<11, // not global. Determines how this is marked in the TLB.
            APP  = (V | AP1 | AP2),
            APPC = (V | AP1 | AP2),
            APPD = (V | AP1 | AP2),
            SYS  = (V | AP1 | AP2),
    ...
    public:
        ...
        Page_Flags(Flags f) : _flags(V | AP1 | AP2) {}
    ...
    public:
        ...
        Phy_Addr phy_address() const {
            return Phy_Addr(indexes((*_pt)[_from]));
        }
...
public:
    ...
    static PD_Entry phy2pde(Phy_Addr frame) { return (frame) | Page_Flags::V; }
